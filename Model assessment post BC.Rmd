...
```{r}
bigcreek <- read_csv("C:/Users/gusta/Box/VICE Lab/RESEARCH/PROJECTS/CERC-WET/Task7_San_Joaquin_Model/pywr_models/data/Upper San Joaquin River/hydrology/historical/Livneh/runoff_aggregated/Big Creek 3 Conduit Outflow Inflow mcm.csv") %>%
  filter(., between(date, as.Date("1984-10-01"), as.Date("2010-09-30")))
bigcreek

bigcreek2 <- read_csv("C:/Users/gusta/Box/VICE Lab/RESEARCH/PROJECTS/CERC-WET/Task7_San_Joaquin_Model/pywr_models/data/Upper San Joaquin River/hydrology/historical/Livneh - Original/runoff_aggregated/Big Creek 3 Conduit Outflow Inflow mcm.csv") %>%
  filter(., between(date, as.Date("1984-10-01"), as.Date("2010-09-30")))
bigcreek2

bigcreek3 <- read_csv("C:/Users/gusta/Desktop/streamflow_cfs.csv")%>%
  select(Date, `USGS 11241800 BIG C PH NO 3 NR SHAVER LK CA`) %>%
  mutate(`USGS 11241800 BIG C PH NO 3 NR SHAVER LK CA` = ((as.numeric(`USGS 11241800 BIG C PH NO 3 NR SHAVER LK CA`)/35.3146667)*86400)/1000000) %>%
  filter(., between(Date, as.Date("1984-10-01"), as.Date("2010-09-30")))
bigcreek3
...
```{r}
bigcreek3 <- read.xlsx("C:/Users/gusta/Desktop/BigCreek3.xlsx", sheetIndex = 1)  %>%
  filter(., between(Date, as.Date("1984-10-01"), as.Date("2010-09-30")))
bigcreek3
model.assess(bigcreek3$BigCreek3_sim, bigcreek3$BigCreek3_obs)

model.assess(bigcreek2$flow, bigcreek3$BigCreek3_obs)
```
...
MRC + annotate("text", as.Date("1987-07-01"), y = 1500, #adding where to add text 
               label = "RSR = 0.55/nPBIAS = 16.45/nNSE = 0.69",  #adding manually results of statistical tests
...
MRC.full + annotate("text", as.Date("1960-07-01"), y = 1700, #adding where to add text 
               label = "RSR = 0.59/nPBIAS = 17.28/nNSE = 0.64",   #adding manually results of statistical tests
               collapse = "/n", hjust = 0, size=4, family= "Times New Roman") +
