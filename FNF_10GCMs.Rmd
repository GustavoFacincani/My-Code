...
Livneh <- read_csv(location) %>%
 filter(date >= as.Date("1950-10-01") & date <= as.Date("2010-09-30"))  %>%
...
GCM <- read_csv(location) %>%
 filter(date >= as.Date("2030-10-01") & date <= as.Date("2090-09-30"))%>%
...
#Mid-century
#Tuolumne RCP 8.5
Livneh_Tuo <- Livneh("C:/Users/gusta/Box/VICE Lab/RESEARCH/PROJECTS/CERC-WET/Task7_San_Joaquin_Model/pywr_models/data/Tuolumne River/hydrology/historical/Livneh/preprocessed/full_natural_flow_daily_mcm.csv")
Livneh_Tuo
...
#Mid-century
#Tuolumne - RCP 4.5
Livneh_Tuo2 <- Livneh("C:/Users/gusta/Box/VICE Lab/RESEARCH/PROJECTS/CERC-WET/Task7_San_Joaquin_Model/pywr_models/data/Tuolumne River/hydrology/historical/Livneh/preprocessed/full_natural_flow_daily_mcm.csv")
Livneh_Tuo2
...
Livneh_USJ <- Livneh("C:/Users/gusta/Box/VICE Lab/RESEARCH/PROJECTS/CERC-WET/Task7_San_Joaquin_Model/pywr_models/data/Upper San Joaquin River/hydrology/historical/Livneh/preprocessed/full_natural_flow_daily_mcm.csv")
Livneh_USJ
...
Livneh_USJ2 <- Livneh("C:/Users/gusta/Box/VICE Lab/RESEARCH/PROJECTS/CERC-WET/Task7_San_Joaquin_Model/pywr_models/data/Upper San Joaquin River/hydrology/historical/Livneh/preprocessed/full_natural_flow_daily_mcm.csv")
Livneh_USJ2
...
RCP45 <- rbind(Mer_RCP45, Tuo_RCP45, Stn_RCP45, USJ_RCP45) %>%
  ...
  group_by(Year, Month, GCM, Basin) %>%
     dplyr::summarize(MonthlyTotal = sum(flow))
RCP45

RCP45$GCM <- factor(RCP45$GCM, levels = c("Livneh (Historical)", "ACCESS1-0", "CMCC-CMS", "MIROC5\u2020", "GFDL-CM3", "CCSM4", "HadGEM2-ES\u2020", "HadGEM2-CC", "CanESM2\u2020", "CESM1-BGC", "CNRM-CM5\u2020"))

RCP45annual <- RCP45 %>%
  group_by(Year, GCM, Basin) %>%
...
RCP85$GCM <- factor(RCP85$GCM, levels = c("Livneh (Historical)", "ACCESS1-0", "CMCC-CMS", "MIROC5\u2020", "GFDL-CM3", "CCSM4",  "HadGEM2-CC", "HadGEM2-ES\u2020","CESM1-BGC", "CanESM2\u2020", "CNRM-CM5\u2020"))

RCP85annual <- RCP85 %>%
  group_by(Year, GCM, Basin) %>%
   dplyr::summarize(AnnualTotal = sum(MonthlyTotal)) %>%
  group_by(Year, GCM, Basin) %>%
  mutate(Mean = median(AnnualTotal),
         variable = GCM) %>%
  filter(!as.numeric(as.character(Year)) > 2010 | !as.numeric(as.character(Year)) < 2031 & !as.numeric(as.character(Year)) > 2090)
RCP85annual
```
...
#extra
RCP85_2 <- RCP85annual %>% 
  group_by(variable, Basin, GCM)%>%
  summarize(`Minimum FNF (mcm)` = min(AnnualTotal, na.rm=TRUE),
            `1st Quintile FNF` = quantile(AnnualTotal, 0.2, na.rm = TRUE),
            #`1st Quartile FNF` = quantile(AnnualTotal, 0.25, na.rm=TRUE),
            `2nd Quintile FNF (mcm)` = quantile(AnnualTotal, 0.4, na.rm = TRUE),
            #`Mean FNF (mcm)` = mean(AnnualTotal, na.rm=TRUE),
            #`Median FNF (mcm)` = median(AnnualTotal, na.rm=TRUE),
            `3th Quintile FNF (mcm)` = quantile(AnnualTotal, 0.6, na.rm = TRUE),
            #`3rd Quartile FNF (mcm)` = quantile(AnnualTotal, 0.75, na.rm=TRUE),
            `4th Quintile FNF (mcm)` = quantile(AnnualTotal, 0.8, na.rm = TRUE))#,
#            AnnualTotal = AnnualTotal)# %>%
#              mutate(Classification = ifelse(AnnualTotal < `1st Quintile FNF`, "1st Quintile FNF",0))
RCP85_2
```
...
ggplot(RCP85annual) + 
  theme_bw(base_size=12, base_family='Times New Roman') + #change font to Times New Roman, 12pt, Bold
    geom_violin(draw_quantiles = c(.25, .5, .75), aes( x = GCM, y = AnnualTotal, fill = GCM)) + #plot monthly observed data in greenish blue
  ...
  labs(title = "Annual Full Natural Flow (2030-2060)",
       subtitle = "RCP 8.5",
    x = element_blank(), 
       y = "Total Outflow (mcm/year)") + #name of x axis
...
png("Annual_FNF_Report_March.png", units ="in", width=11.5, height=4.5, res = 300)
```
...
All_Scenarios$GCM <- factor(All_Scenarios$GCM, levels = c("Livneh (Historical)", "ACCESS1-0", "CMCC-CMS", "MIROC5\u2020", "GFDL-CM3", "CCSM4", "HadGEM2-CC", "HadGEM2-ES\u2020","CESM1-BGC", "CanESM2\u2020", "CNRM-CM5\u2020"))

#All_Scenarios <- All_Scenarios %>%
#  mutate(Dateof50 = as.Date(paste("1950-",format(as.Date(Date_50pct_TotalQ), "%m"),"-", format(as.Date(Date_50pct_TotalQ), "%d"), sep="")))
#All_Scenarios
```
...
Merced <- All_Scenarios %>%
  filter(Basin == "Merced River")

range(Merced$DoY_50pct_TotalQ) #Merced 111-240 (Jan 19 - May 28), Tuo 77-257 (Dec 16 - Jun 13), Stn 68-237 (Dec 07 - May 25), USJ 92-273 (Dec 31 - Jun 30)

All_Scenarios %>%
  filter(Basin == "Merced River",
         DoY_50pct_TotalQ == "50" | DoY_50pct_TotalQ == "278")

```

```{r}
All_Scenarios2 <- All_Scenarios%>%
  mutate(Period = ifelse(Year <2011, "1981-2010", ifelse(Year > 2060, "2061-2090", "2031-2060")))
All_Scenarios2
```

```{r}
Density_plot <- function(data, title, basin, Legend) {
   ...
   # scale_color_brewer(palette = "RdYlBu")+
    ...
    scale_x_continuous(expand = c(0, NA),
                # limits = c(as.Date("1952-01-15"), as.Date("1952-05-30")))+
                       ...
                       limits = c(15,288), #From Jan 1 to Jul 15
                       breaks = c(31, 62, 93, 122, 152, 
                                  183, 213, 245, 274),#, 305),
                       labels = c("Nov 1","Dec 1","Jan 1","Feb 1", "Mar 1",
...
#  facet_wrap(~Period, ncol=1, scales = "fixed",  strip.position = "left")
  
}  
```

```{r}
Density_plot(All_Scenarios  %>% filter(RCP == "RCP 8.5")
             , element_blank(),  element_blank(), "RCP 8.5") +
  png("Allbasins_CT_FNF_RCP85_10GCMs_Report_March.png", units ="in", width=5.2, height=7, res = 300)
```
```{r}
Density_plot(All_Scenarios  %>% filter(RCP == "RCP 4.5")
             , element_blank(),  element_blank(), "RCP 4.5") +
  png("Allbasins_CT_FNF_RCP45_Report_March.png", units ="in", width=5.2, height=7, res = 300)
```


```{r}
Stn_RCP_85$GCM <- factor(Stn_RCP_85$GCM, levels = c("Livneh (Historical)","MIROC5\u2020", "ACCESS1-0", "GFDL‐CM3","CMCC-CMS",  "HadGEM2-ES\u2020", "CCSM4", "CESM1-BGC", "HadGEM2‐CC","CanESM2\u2020", "CNRM-CM5\u2020", "Ensemble"))

Density_plot(All_Scenarios %>%
  filter(Basin == "Tuolumne River"), element_blank(), "Tuolumne River", "RCP 8.5") +
  png("Tuo_CT_FNF_RCP85_Report_March.png", units ="in", width=6, height=3.5, res = 300)
```

```{r}
Density_plot(All_Scenarios %>% #
  filter(Basin == "Merced River" & RCP == "RCP 8.5") %>%
  filter(Year < 2030 | Year > 2061), element_blank(), "Merced River", "RCP 8.5") +
  png("Merced_CT_FNF_RCP85_ES240_2061-2090.png", units ="in", width=6, height=3.5, res = 300)

#filter(Basin == "Merced River" & Year < 2061 & RCP == "RCP 8.5")


#All_Scenarios %>% #
#  filter(Basin == "Merced River" & RCP == "RCP 8.5") %>%
#  filter(Year < 2030 | Year > 2061)
```
...
All_basins$GCM <- factor(All_basins$GCM, levels = c("Livneh (Historical)", "ACCESS1-0", "CMCC-CMS", "MIROC5\u2020", "GFDL-CM3", "CCSM4", "HadGEM2-CC", "HadGEM2-ES\u2020","CESM1-BGC", "CanESM2\u2020", "CNRM-CM5\u2020"))
All_basins
```
...
readr::write_csv(all_FNF2, "C:/Users/gusta/Desktop/PhD/Classes/ES207/Allbasins_Stats_FNF_10GCMs_RCP85_Report_March.csv")
```