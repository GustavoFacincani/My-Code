...
```{r}
# Define the base path for the .nc files (i.e., where the climate data is stored)
netcdf_path <- "C:/Users/gusta/Box/VICE Lab/RESEARCH/PROJECTS/CERC-WET/Task7_San_Joaquin_Model/Pywr models/data/common/hydrology/NetCDF files/Total Runoff/CCSM4/rcp85"
...
extract_climate_data <- function(climate, varid, coeffs) {
  ...
  GCMs <- c("ACCESS1‐0","CCSM4")
  for(GCM in GCMs){  
    
    subwats <- tail(names(coeffs), -2) # get subwats from coefficients table (this could be done manually)
    
    climate_path <- "C:/Users/gusta/Box/VICE Lab/RESEARCH/PROJECTS/CERC-WET/Task7_San_Joaquin_Model/Pywr models/data/common/hydrology/NetCDF files/Total Runoff/" #paste(netcdf_path, "total runoff", climate, sep="/")
    nc_files <- list.files(path = paste(climate_path, GCM,"/rcp85/", sep =""), pattern = "//.nc$")
    ...
    for (nc_file in nc_files) {
      ...
      write.csv(big_df, paste0("C:/Users/gusta/Box/VICE Lab/RESEARCH/PROJECTS/CERC-WET/Task7_San_Joaquin_Model/Pywr models/data/common/hydrology/NetCDF files/DataExtracted","/", GCM,"_allbasins.csv", sep =""), row.names = FALSE)
...
extract_climate_data(climate, varid, coeffs)
```