...
```{r}
Storage1 <- cbind(Livneh_Stor_Mer, Livneh_Stor_Stn[-1], Livneh_Stor_Tuo[-1], Livneh_Stor_USJ[-1])
...
x <-  lapply(names(basins), NonMissingStartEnd, date.col = 'node', df=basins)
Storage_TimeSpan <- do.call(rbind,x)
Storage_TimeSpan

#get time span between to dates, 
Storage2 <- reshape2::melt(Storage1, id = "node")
Storage2


```
EIA_manipulated <- merge(Storage2, Storage_TimeSpan, by.x = "variable", by.y = "my.col")
EIA_manipulated
