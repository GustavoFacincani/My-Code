...
```{r}
#Total capacity is 2985 mcm, 500 mcm is dead pool, so 1/3 of it is 828.333 mcm 
OperCap_Stn <- rbind(Stor_Stn1, Livneh_Stn_Stor) %>%
  mutate(Month = as.factor(format(as.Date(node, format = "%B"), "%b")),
                           Year = water_year(node, origin = "usgs"),
         Scenario = as.factor("RCP 4.5"),
         DeadPool = as.numeric(500),
         OperatingCapacity = (value - DeadPool),
         Count = ifelse(OperatingCapacity > 828.333, 0,1))
OperCap_Stn
```
...
Basins_Capacity %>%
  filter(variable == "New Melones Lake") %>%
ggplot() +
